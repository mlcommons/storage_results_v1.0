...
run() {
	...
	config_name=$(get_config_file $workload $accelerator_type)
	prefixed_array=$(add_prefix_params ${params[@]})
	mpirun -hosts $hosts -np $num_accelerators cgexec -g memory:memory_mlperf python3 dlio_benchmark/dlio_benchmark/main.py --config-path=$CONFIG_PATH workload=$config_name ++workload.workflow.generate_data=False ++workload.workflow.train=True ${prefixed_array[@]} ${EXTRA_PARAMS[@]}
