...
# A100 runs
for i in {1..5}; do
  run_name="run${i}_1c_${workload}_a100_25a"
  ...
  echo "Clearing cache before run."
    ...
    sleep 30
  ./benchmark.sh run -s $server -w $workload -g a100 -n 24 -r /mnt/weka/storage/storage_results_v1.0/cache-cleared/closed/WEKA/results/weka-1-client-dual-socket-56-cores/unet3d-a100/run${i} --param dataset.num_files_train=91000 --param dataset.num_subfolders_train=1000 --param dataset.data_folder=/mnt/weka/storage/unet3d_data_a100 --param reader.read_threads=32 --param reader.prefetch_size=0 --param reader.transfer_size=0
  if [ $? -ne 0 ]; then
