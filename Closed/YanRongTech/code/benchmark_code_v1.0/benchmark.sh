...
datagen() {
	...
	mpirun --allow-run-as-root --host $hosts -np $parallel python3 dlio_benchmark/dlio_benchmark/main.py --config-path=$CONFIG_PATH workload=$config_name ++workload.workflow.generate_data=True ++workload.workflow.train=False ${prefixed_array[@]} ${EXTRA_PARAMS[@]}
}
...
run() {
	...
	config_name=$(get_config_file $workload $accelerator_type)
	prefixed_array=$(add_prefix_params ${params[@]})
	mpirun --allow-run-as-root --host $hosts -np $num_accelerators python3 dlio_benchmark/dlio_benchmark/main.py --config-path=$CONFIG_PATH workload=$config_name ++workload.workflow.generate_data=False ++workload.workflow.train=True ${prefixed_array[@]} ${EXTRA_PARAMS[@]}
